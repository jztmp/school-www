...
function displayInfo(data,num){
	var dt = new Date(data['UTIME'].replace(/-/g,"/"));
	getInfo(dt);
	...
	return (num? data['BUS_NUMBER'] : '') + ' ' + distance + ' miles' + ' ' + nowStr;
}
...
var api = {
    ...
    getODB2GPS : function(latitude,longitude){
        
        var url2 = webPath + "/api/SCHOOL_SERVICE/getodb2Date/{}?LATITUDE=" 
        + latitude.toString(10) + "&LONGITUDE=" + longitude.toString(10);
        //window.location.href = encodeURIComponent(url2);
        
        $.getJSON(url2, null, function (data, textStatus){
        	$("#htime").html('Tracking...');
        	$("#heartNumber").html('No data');
			...
			var display = displayInfo(list[0],false);
			if (display){
				$("#htime").html('<a href="#" onclick="api.showMapodb2(' + list[0]['LATITUDE'] + ',' + list[0]['LONGITUDE']+')">' + display + '</a>');
				$('#heartNumber').html(list[0]['BUS_NUMBER']);
			}
			
            var content = '';
            $("#contentList1").html('');
            for (var i=1;i<list.length;i++){
            	display = displayInfo(list[i],true);
            	if (!display){
            		continue;
            	}
                content = content + "<li>";
				// content = content + "<div class=\"listtitle\">"+list[i]['PULSECOUNT']+"<span class=\"timestyle\">"+list[i]['DATE']+"</span></div>";
                content = content + "<div class=\"listtitle\">"+ 
                "<a href=\"#\" onclick=\"api.showMapodb2("+list[i]['LATITUDE']+","+list[i]['LONGITUDE']+")\">" 
                + display +" </a></div>";
                content = content + "</li>";
            }
        ...
        /*var url = webPath+"/api/SCHOOL_SERVICE/getodb2CurrentDate/{'PID':'"+PID+"'}";
