    ...
    getODB2GPS : function(imei){
		...
		$.getJSON(url, null, function (data, textStatus){
			if(data['result']){
					...
					*/
					$("#nameDiv").html(data['result']['PARENTS_NAME']+" Bus");
					$("#telDiv").html(data['result']['TEL']);
					$("#heartNumber").html(data['result']['BUS_NUMBER_TO'] ? 'SUB:' + data['result']['BUS_NUMBER_TO']: data['result']['BUS_NUMBER']);
					var sms = data['result']['SMS'];
					var tmp = '';
					while (sms.length > 11){
						tmp += sms.substring(0,11) + '<br>';
						sms = sms.substring(11);
					}
					tmp += sms;
					$("#sms").html(tmp.length > 0 ? tmp : sms);
